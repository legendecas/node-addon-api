...
T Maybe<T>::FromJust() const {
  NAPI_CHECK(IsJust(), "Napi::Maybe::FromJust", "Maybe value is Nothing.");
  return _value;
}
...
inline MaybeOrValue<Symbol> Symbol::WellKnown(napi_env env,
  ...
  Value symbol_obj;
  Value symbol_value;
  if (Napi::Env(env).Global().Get("Symbol").To(&symbol_obj) &&
      symbol_obj.As<Object>().Get(name).To(&symbol_value)) {
...
#ifdef NODE_ADDON_API_ENABLE_MAYBE
  if (result.IsJust()) {
    return Just(scope.Escape(result.ToChecked()));
  }
  return result;
#else
...
#ifdef NODE_ADDON_API_ENABLE_MAYBE
  if (result.IsJust()) {
    return Just(scope.Escape(result.ToChecked()).As<Object>());
  }
  return result;
#else
  if (scope.Env().IsExceptionPending()) {
