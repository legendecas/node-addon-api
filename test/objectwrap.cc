...
Napi::Value StaticGetter(const Napi::CallbackInfo& /*info*/) {
  return FromMaybe(testStaticContextRef.Value().Get("value"));
}
...
Napi::Value TestStaticMethod(const Napi::CallbackInfo& info) {
  std::string str = MaybeToChecked(info[0].ToString());
...
public:
  ...
  void Setter(const Napi::CallbackInfo& /*info*/, const Napi::Value& value) {
    value_ = MaybeToChecked(value.ToString());
  }
  ...
  Napi::Value Iterator(const Napi::CallbackInfo& info) {
    ...
    return FromMaybe(
        MaybeToChecked(array.Get(MaybeToChecked(
  ...
  void TestVoidMethodT(const Napi::CallbackInfo &info) {
    value_ = MaybeToChecked(info[0].ToString());
  }
  ...
  static void TestStaticVoidMethodT(const Napi::CallbackInfo& info) {
    s_staticMethodText = MaybeToChecked(info[0].ToString());
  }
    ...
    exports.Set(
        ...
        DefineClass(
            ...
            "Test",
            {
                ...
                // conventions
                InstanceAccessor(
                    MaybeToChecked(Napi::Symbol::WellKnown(env, "toStringTag")),
                    &Test::ToStringTag,
                    nullptr,
                    napi_enumerable),
                InstanceMethod(
                    MaybeToChecked(Napi::Symbol::WellKnown(env, "iterator")),
                    &Test::Iterator,
                    napi_default),
