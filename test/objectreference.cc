...
Value GetFromGetter(const CallbackInfo& info) {
  ...
  if (info[0].As<String>() == String::New(env, "weak")) {
    if (weak.IsEmpty()) {
      ...
      if (info[1].IsString()) {
        return FromMaybe(weak.Get(info[1].As<String>().Utf8Value()));
      } else if (info[1].IsNumber()) {
        return FromMaybe(weak.Get(info[1].As<Number>().Uint32Value()));
      }
    }
  } else if (info[0].As<String>() == String::New(env, "persistent")) {
    if (info[1].IsString()) {
      return FromMaybe(persistent.Get(info[1].As<String>().Utf8Value()));
    } else if (info[1].IsNumber()) {
      return FromMaybe(persistent.Get(info[1].As<Number>().Uint32Value()));
    }
  } else {
    if (info[0].IsString()) {
      return FromMaybe(reference.Get(info[0].As<String>().Utf8Value()));
    } else if (info[0].IsNumber()) {
      return FromMaybe(reference.Get(info[0].As<Number>().Uint32Value()));
    }
  }
...
Value GetCastedFromGetter(const CallbackInfo& info) {
  ...
  if (info[0].As<String>() == String::New(env, "weak")) {
    if (casted_weak.IsEmpty()) {
      ...
      return FromMaybe(casted_weak.Get(info[1].As<Number>()));
    }
  } else if (info[0].As<String>() == String::New(env, "persistent")) {
    return FromMaybe(casted_persistent.Get(info[1].As<Number>()));
  } else {
    return FromMaybe(casted_reference.Get(info[1].As<Number>()));
  }
}
